...
function WriteAddress() { 
    ...
    if($filtered['dong'] == "") {
        ...
        $query = "select * from gangnam where 지역코드 = {$filtered['gu']} AND 전용면적 <= '{$filtered['size']}' AND 거래금액 <= '{$filtered['price']}' limit 200;";
    } else {
        // 구-동까지 모두 선택했을 경우. 해당 구의 특정 동의 아파트 조회 (100개 조회)
        $query = "select * from gangnam where 지역코드 = {$filtered['gu']} AND 법정동 = '{$filtered['dong']}' AND 전용면적 <= '{$filtered['size']}' AND 거래금액 <= '{$filtered['price']}' limit 100;";
    }
